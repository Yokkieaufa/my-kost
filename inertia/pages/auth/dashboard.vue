...
import { Head, Link, router } from '@inertiajs/vue3'; // Ensure 'router' is imported for client-side routing
...
const performSearch = () => {
    ...
    // but the computed property `filteredKosts` already reacts to changes in search and filter inputs.
};

const showScrollToTop = ref(false);

const openWhatsApp = () => {
    // === PERUBAHAN 1: Pesan WhatsApp ===
    const whatsappMessage = encodeURIComponent("permisi saya ingin menanyakan lebih mendalam tentang seputar kost");
    window.open(`https://wa.me/6288216795013?text=${whatsappMessage}`, '_blank');
};
...
/**
 * Handles the logout action by making a POST request to the '/logout' route.
 * In Inertia.js, this typically sends a request to your backend's logout endpoint,
 * which then clears the session and redirects the user.
 */
    ...
    <div class="min-h-screen flex flex-col bg-gray-100">
        <header class="bg-green-600 py-3 md:py-4 shadow-md">
            <nav class="container mx-auto px-4 flex items-center justify-between">
                ...
                <div class="flex items-center space-x-4 md:space-x-8 text-sm md:text-base">
                    ...
                    <button @click="handleLogout" class="bg-red-500 hover:bg-red-600 text-white font-semibold px-4 py-2 rounded-lg">
                        Logout
                    </button>
                </div>
            </nav>
        ...
        <main class="flex-grow">
            <section class="relative bg-gradient-to-r from-green-700 to-green-500 text-white py-16 md:py-24">
                <img src="/public/hero.png" alt="Hero Background" class="absolute inset-0 w-full h-full object-cover opacity-30">
